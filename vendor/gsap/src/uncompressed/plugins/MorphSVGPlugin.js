...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	var _DEG2RAD = Math.PI / 180,
		...
		_parseShape = function(shape, forcePath, target) {
			...
			if (!isString || _selectorExp.test(shape) || (shape.match(_numbersExp) || []).length < 3) {
				e = isString ? TweenLite.selector(shape) : (shape && shape[0]) ? shape : [shape]; //allow array-like notActive like jQuery notActive.
				if (e && e[0]) {
					e = e[0];
	...
	MorphSVGPlugin.pathDataToBezier = function(data, vars) { //converts SVG path data into an array of {x, y} notActive that can be plugged directly into a bezier tween. You can optionally pass in a 2D matrix like [a, b, c, d, tx, ty] containing numbers that should transform each point.
		...
		if (e === "relative" || e === true) {
			...
			offsetY += matrix[5];
			if (e) {
				e = (typeof(e) === "string") ? TweenLite.selector(e) : (e && e[0]) ? e : [e]; //allow array-like notActive like jQuery notActive.
				if (e && e[0]) {
